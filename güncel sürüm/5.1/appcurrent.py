...
#DamagedReturnButton:hover { background-color: #FECACA; }
#DangerButton {
...
#DangerButton:hover { background-color: #FEE2E2; }
...
#DamagedReturnButton:hover { background-color: #991B1B; }
#DangerButton {
...
#DangerButton:hover { background-color: #7F1D1D; color: #FFFFFF; }
QPushButton[class="nav-btn"] {
    background-color: transparent;
    border: none;
    text-align: left;
...
def require_active_user(db, parent):
    """
    Kayıtsız (kullanıcı girişi yapılmamış) işlem yapılmasını engeller.
    Sol menüden 'Kullanıcı Girişi' ile bir kullanıcı seçilmemişse
    uyarı gösterir ve False döner; işlem iptal edilmelidir.
    """
    if not db.current_user_id:
        QMessageBox.warning(
            parent,
            "Kullanıcı Girişi Gerekli",
            "Bu işlemi yapabilmek için önce sol menüden "
            "'Kullanıcı Girişi' ile giriş yapmalısınız.\n\n"
            "Kayıtsız (kimin yaptığı belli olmayan) işlem yapılamaz."
        )
        return False
    return True
...
class Database:
    ...
    def create_tables(self):
        ...
        self.execute("""
            CREATE TABLE IF NOT EXISTS loans (
                ...
                returned_at TEXT,
                notes TEXT,
                return_status TEXT DEFAULT 'Normal',
                damage_notes TEXT,
    ...
    def add_user(self, user_number, name, role="Kullanıcı"):
        user_number = (user_number or "").strip()
        name = (name or "").strip()

        if not user_number:
            raise ValueError("Kullanıcı numarası boş olamaz.")

        if not name:
            raise ValueError("Kullanıcı adı boş olamaz.")

        existing_number = self.fetchone("""
            SELECT id FROM system_users WHERE user_number = ?
        """, (user_number,))

        if existing_number:
            raise ValueError("Bu kullanıcı numarası zaten kayıtlı.")

        cur = self.execute("""
            INSERT INTO system_users (user_number, username, role, status)
            VALUES (?, ?, ?, 'Aktif')
        """, (user_number, name, role))
    ...
    def get_user_by_number(self, user_number):
        return self.fetchone("""
            ...
            WHERE user_number = ? AND status = 'Aktif'
        """, ((user_number or "").strip(),))
    ...
    def update_book(
        ...
        self.log("Kitap bilgileri düzenlendi (admin)", "book", book_id)
    ...
    def get_damaged_or_lost_copies(self):
        return self.fetchall("""
            SELECT
                c.*,
                b.title,
            ...
            FROM book_copies c
            JOIN books b ON b.id = c.book_id
            WHERE c.status IN ('Hasarlı', 'Kayıp', 'Arşivde', 'Bakımda')
            ORDER BY c.status, b.title COLLATE NOCASE
        """)
    ...
    def update_copy_status(self, copy_id, new_status):
        ...
        self.log(
            f"Nüsha durumu '{new_status}' olarak güncellendi",
            "copy",
            copy_id
        )
    ...
    def update_copy(
        ...
        self.log(
            "Nüsha bilgileri düzenlendi (admin)",
            "copy",
            copy_id
        )

    def get_copies_by_title(self, text):
        text = (text or "").strip()
        ...
        return self.fetchall("""
            ...
            ORDER BY
                CASE
                    WHEN c.status = 'Mevcut' THEN 0
                    ELSE 1
                END,
                b.title COLLATE NOCASE
        """, (like, like))

    def get_activity_logs(self, limit=500):
        return self.log_fetchall("""
            SELECT *
            FROM activity_logs
            ORDER BY id DESC
            LIMIT ?
    ...
    def update_member(
        ...
        self.log("Üye bilgileri düzenlendi (admin)", "member", member_id)
    ...
    def return_book(self, loan_id, damaged=False, return_notes="", damage_notes=""):
        ...
        return_status = "Hasarlı" if damaged else "Normal"

        combined_notes = loan["notes"] or ""
        return_notes = (return_notes or "").strip()

        if return_notes:
            combined_notes = (
                (combined_notes + "\n" if combined_notes else "")
                + f"[İade Notu] {return_notes}"
            )

        damage_notes = (damage_notes or "").strip() if damaged else None
        ...
        new_copy_status = "Hasarlı" if damaged else "Mevcut"

        if damaged and damage_notes:
            # Hasar açıklamasını nüshanın kendi notuna da işliyoruz,
            # aksi halde "Hasarlı / Kayıp Nüshalar" listesinde görünmüyordu.
            existing_copy = self.get_copy(loan["book_copy_id"])
            existing_copy_notes = (
                (existing_copy["notes"] or "") if existing_copy else ""
            )

            combined_copy_notes = (
                (existing_copy_notes + "\n" if existing_copy_notes else "")
                + f"[Hasar - {returned_at}] {damage_notes}"
            )
        ...
        self.log(
            "Kitap hasarlı iade alındı" if damaged else "Kitap iade alındı",
            "loan",
            loan_id
        )
...
# =========================================================
# KİTAP DÜZENLEME PENCERESİ (Admin)
# =========================================================

class BookEditDialog(QDialog):
    """
    Admin şifresi ile açılan, bir kitabın TÜM bilgilerinin
    düzenlenebildiği pencere (ISBN, ad, yazar, yayınevi, yıl,
    baskı, dil, kategori, açıklama, durum).
    """
    ...
    def __init__(self, db, book_id, parent=None):
        ...
        self.setWindowTitle("Kitap Bilgilerini Düzenle (Admin)")
        self.resize(500, 600)

        layout = QVBoxLayout(self)

        info = QLabel(
            "Admin yetkisiyle kitabın tüm bilgilerini "
            "düzenleyebilirsiniz."
        )
        ...
        self.year_edit.setRange(0, 3000)
        ...
        self.status_combo.addItems(["Aktif", "Arşivde"])

        form.addRow("ISBN / Barkod:", self.isbn_edit)
        form.addRow("Kitap Adı:", self.title_edit)
        form.addRow("Yazar:", self.author_edit)
        form.addRow("Yayınevi:", self.publisher_edit)
        form.addRow("Yayın Yılı:", self.year_edit)
        form.addRow("Baskı:", self.edition_edit)
        form.addRow("Dil:", self.language_edit)
        form.addRow("Kategori:", self.category_edit)
        form.addRow("Açıklama / Not:", self.description_edit)
    ...
    def load_data(self):
        ...
        self.year_edit.setValue(book["publication_year"] or 0)
        self.edition_edit.setText(book["edition"] or "")
        self.language_edit.setText(book["language"] or "")
        self.category_edit.setText(book["category"] or "")
        self.description_edit.setPlainText(book["description"] or "")

        index = self.status_combo.findText(book["status"] or "Aktif")
        if index >= 0:
    ...
    def save(self):
        ...
        try:
            self.db.update_book(
                ...
                self.year_edit.value() or None,
                self.edition_edit.text(),
                self.language_edit.text(),
...
class CopyEditDialog(QDialog):
    """
    Admin şifresi ile açılan, bir fiziksel nüshanın (kitap
    numarası / barkod, raf, durum, not) tüm bilgilerinin
    düzenlenebildiği pencere.
    """

    def __init__(self, db, copy_id, parent=None):
        super().__init__(parent)

        self.db = db
        self.copy_id = copy_id

        self.setWindowTitle("Nüsha Bilgilerini Düzenle (Admin)")
        self.resize(420, 320)
        ...
        self.book_number_edit = QLineEdit()
        ...
        form.addRow("Kitap Numarası / Barkod:", self.book_number_edit)
        form.addRow("Raf:", self.shelf_edit)
        form.addRow("Durum:", self.status_combo)
    ...
    def load_data(self):
        ...
        copy = self.db.get_copy(self.copy_id)

        if not copy:
            return

        self.book_number_edit.setText(copy["book_number"] or "")
        self.shelf_edit.setText(copy["shelf"] or "")
        self.notes_edit.setPlainText(copy["notes"] or "")

        index = self.status_combo.findText(copy["status"] or "Mevcut")
        if index >= 0:
            self.status_combo.setCurrentIndex(index)
    ...
    def save(self):
        ...
        try:
            self.db.update_copy(
                self.copy_id,
                self.book_number_edit.text(),
                self.shelf_edit.text(),
                self.status_combo.currentText(),
...
class BookDetailsDialog(QDialog):
    ...
    def __init__(self, db, book_id, parent=None):
        ...
        edit_book_button = QPushButton(
            "✏  Kitap Bilgilerini Düzenle (Admin)"
        )

        edit_book_button.clicked.connect(
            self.edit_book
        )
        ...
        edit_copy_button = QPushButton(
            "✏  Seçili Nüshayı Düzenle (Admin)"
        )
    ...
    def edit_book(self):
        ...
        edit_dialog = BookEditDialog(self.db, self.book_id, self)
    ...
    def edit_copy(self):
        ...
        row = self.copies_table.currentRow()

        if row < 0:
            QMessageBox.warning(
                self,
                "Uyarı",
                "Düzenlenecek nüshayı (satırı) seçin."
            )
            return

        copy_id = int(
            self.copies_table.item(row, 0).text()
        )
        ...
        edit_dialog = CopyEditDialog(self.db, copy_id, self)

        if edit_dialog.exec() == QDialog.Accepted:
            QMessageBox.information(
                self,
                "Başarılı",
                "Nüsha bilgileri güncellendi."
            )
            self.load_data()

    def delete_book(self):

        if not require_active_user(self.db, self):
            return

        answer = QMessageBox.question(
            self,
            "Kitabı Sil",
            "Bu kitap arşivlenecek (kalıcı olarak silinmeyecek, "
            "takip amacıyla saklanacaktır).\n\n"
            "Devam etmek için önce admin girişi yapmanız gerekiyor. "
            "Devam edilsin mi?"
        )
        ...
        try:
            self.db.archive_book(self.book_id)

            QMessageBox.information(
                self,
                "Başarılı",
                "Kitap arşivlendi (soft delete)."
            )
    ...
    def load_data(self):
        ...
        self.info_label.setText(
            ...
            f"<b>Durum:</b> {book['status']}<br>"
            f"<b>Açıklama / Not:</b> "
            f"{(book['description'] or '-').replace(chr(10), '<br>')}"
        )
...
# =========================================================
# ÜYE DÜZENLEME PENCERESİ (Admin)
# =========================================================

class MemberEditDialog(QDialog):
    """
    Admin şifresi ile açılan, bir üyenin TÜM bilgilerinin
    düzenlenebildiği pencere (üye no, ad, soyad, telefon,
    e-posta, durum, not).
    """
    ...
    def __init__(self, db, member_id, parent=None):
        ...
        self.setWindowTitle("Üye Bilgilerini Düzenle (Admin)")
        self.resize(450, 400)

        layout = QVBoxLayout(self)

        info = QLabel(
            "Admin yetkisiyle üyenin tüm bilgilerini "
            "düzenleyebilirsiniz."
        )
        info.setWordWrap(True)
        layout.addWidget(info)
        ...
        form.addRow("Üye Numarası:", self.member_number_edit)
        form.addRow("Ad:", self.first_name_edit)
        form.addRow("Soyad:", self.last_name_edit)
    ...
    def load_data(self):
        ...
        self.member_number_edit.setText(member["member_number"] or "")
        self.first_name_edit.setText(member["first_name"] or "")
        self.last_name_edit.setText(member["last_name"] or "")
        self.phone_edit.setText(member["phone"] or "")
        self.email_edit.setText(member["email"] or "")
        self.notes_edit.setPlainText(member["notes"] or "")

        index = self.status_combo.findText(member["status"] or "Aktif")
        if index >= 0:
            self.status_combo.setCurrentIndex(index)
...
class MemberDetailsDialog(QDialog):
    ...
    def __init__(self, db, member_id, parent=None):
        ...
        edit_member_button = QPushButton(
            "✏  Üye Bilgilerini Düzenle (Admin)"
        )

        edit_member_button.clicked.connect(
            self.edit_member
        )

        delete_member_button = QPushButton(
            "🗑  Üyeyi Sil (Pasifleştir)"
        )

        delete_member_button.setObjectName("DangerButton")

        delete_member_button.clicked.connect(
            self.delete_member
        )
    ...
    def edit_member(self):
        ...
        edit_dialog = MemberEditDialog(self.db, self.member_id, self)
    ...
    def load_data(self):
        ...
        self.info_label.setText(
            ...
            f"<b>Durum:</b> {member['status']}<br>"
            f"<b>Not:</b> "
            f"{(member['notes'] or '-').replace(chr(10), '<br>')}"
        )
    ...
    def delete_member(self):
        ...
        answer = QMessageBox.question(
            self,
            "Üyeyi Sil",
            "Bu üye pasifleştirilecek (kalıcı olarak silinmeyecek, "
            "takip amacıyla saklanacaktır).\n\n"
            "Devam etmek için önce admin girişi yapmanız gerekiyor. "
            "Devam edilsin mi?"
        )
        ...
        try:
            self.db.deactivate_member(self.member_id)

            QMessageBox.information(
                self,
                "Başarılı",
                "Üye pasifleştirildi (soft delete)."
            )
...
class UserNumberLoginDialog(QDialog):
    """Sidebar'daki 'Kullanıcı Girişi' - şifresiz, sadece numara ile."""
    ...
    def __init__(self, parent=None):
        ...
        layout.addWidget(
            QLabel("İşlemi yapan kullanıcının numarasını giriniz:")
        )

        self.number_edit = QLineEdit()
        self.number_edit.setPlaceholderText("Kullanıcı Numarası...")
        self.number_edit.returnPressed.connect(self.try_accept)
...
class LibraryApp(QMainWindow):
    ...
    def create_sidebar(self):
        ...
        # ---- İşlemi yapan kullanıcı (numara ile giriş, şifresiz) ----
        ...
        self.active_user_status_label = QLabel("Giriş yapılmadı")
        self.active_user_status_label.setWordWrap(True)
        layout.addWidget(self.active_user_status_label)

        self.user_login_button = QPushButton("🔑  Kullanıcı Girişi")
        self.user_login_button.clicked.connect(
            self.handle_user_login_button
        )
    ...
    def handle_user_login_button(self):
        ...
        if self.db.current_user_id:
            ...
            answer = QMessageBox.question(
                self,
                "Çıkış Yap",
                "Mevcut kullanıcı oturumunu kapatmak istiyor musunuz?"
            )
        ...
        dialog = UserNumberLoginDialog(self)

        if dialog.exec() != QDialog.Accepted:
            return

        user = self.db.get_user_by_number(dialog.user_number)

        if not user:
            QMessageBox.warning(
                self,
                "Bulunamadı",
                "Bu numarayla eşleşen aktif bir kullanıcı bulunamadı."
            )
            return
    ...
    def update_active_user_label(self):
        ...
        if self.db.current_user_id:
            ...
            if user:
                self.active_user_status_label.setText(
                    f"👤 {user['username']} (No: {user['user_number']})"
                )
                self.user_login_button.setText("🚪  Çıkış Yap")
                return

        self.active_user_status_label.setText("Giriş yapılmadı")
        self.user_login_button.setText("🔑  Kullanıcı Girişi")
    ...
    def create_add_book_tab(self):
        ...
        add_book_lookup_button = QPushButton(
            "🔍  ISBN / Barkod ile Kitap Bilgisi Ara"
        )

        add_book_lookup_button.clicked.connect(
            self.open_isbn_lookup
        )

        layout.addWidget(
            add_book_lookup_button
        )
    ...
    def create_loan_tab(self):
        ...
        self.loan_book_search_edit.setPlaceholderText(
            "ISBN / barkod / Kitap Numarası okutun ya da "
            "kitap adını girin..."
        )
    ...
    def search_loan_book(self):
        ...
        if copy:
            ...
            # Barkod / ISBN ile hiçbir sonuç bulunamadıysa,
            # girilen değeri kitap adı veya yazar olarak
            # aratıyoruz (isimle arama).
            if not copies:
                copies = self.db.get_copies_by_title(
                    value
                )
    ...
    def loan_copy_selected(self):
        ...
        if copy:
            ...
            self.loan_selected_book_label.setText(
                f"Seçili kitap: "
                f"Kitap Adı: {copy['title']} | "
                f"Kitap No (Barkod): {copy['book_number'] or '-'} | "
                f"Durum: {copy['status']}"
            )
    ...
    def create_return_tab(self):
        ...
        self.return_notes_edit = QTextEdit()
        self.return_notes_edit.setMaximumHeight(60)
        self.return_notes_edit.setPlaceholderText(
            "İade ile ilgili genel not (opsiyonel)..."
        )

        self.return_damage_edit = QTextEdit()
        self.return_damage_edit.setMaximumHeight(60)
        self.return_damage_edit.setPlaceholderText(
            "Yalnızca hasarlı iade için: hasarın açıklaması..."
        )

        notes_layout.addRow(
            "Not:",
            self.return_notes_edit
        )

        notes_layout.addRow(
            "Hasar Açıklaması:",
            self.return_damage_edit
        )
        ...
        return_buttons_layout = QHBoxLayout()
        ...
        return_button.clicked.connect(
            lambda: self.return_selected_loan(damaged=False)
        )

        damaged_return_button = QPushButton(
            "🔧  HASARLI İADE AL"
        )

        damaged_return_button.setMinimumHeight(
            45
        )

        damaged_return_button.setObjectName("DamagedReturnButton")

        damaged_return_button.clicked.connect(
            lambda: self.return_selected_loan(damaged=True)
        )

        return_buttons_layout.addWidget(
            return_button
        )

        return_buttons_layout.addWidget(
            damaged_return_button
        )

        current_layout.addLayout(
            return_buttons_layout
        )
    ...
    def return_selected_loan(self, damaged=False):
        ...
        return_notes = self.return_notes_edit.toPlainText().strip()
        damage_notes = self.return_damage_edit.toPlainText().strip()

        if damaged and not damage_notes:
            QMessageBox.warning(
                self,
                "Uyarı",
                "Hasarlı iade için hasar açıklaması girin."
            )
            return

        confirm_text = (
            "Seçili kitabı HASARLI olarak iade almak "
        ...
        try:
            ...
            QMessageBox.information(
                self,
                "Başarılı",
                "Kitap hasarlı olarak iade alındı." if damaged
                else "Kitap iade alındı."
            )

            self.return_notes_edit.clear()
            self.return_damage_edit.clear()
            ...
            self.return_book_result_label.setText(
                "Kitap hasarlı olarak iade edildi." if damaged
                else "Kitap iade edildi."
            )
    ...
    # =====================================================
    # HASARLI / KAYIP NÜSHALAR
    # =====================================================

    def create_damaged_lost_tab(self):

        self.damaged_lost_tab = QWidget()

        layout = QVBoxLayout(
            self.damaged_lost_tab
        )

        title = QLabel(
            "Hasarlı / Kayıp Nüshalar"
        )
        ...
        info_label = QLabel(
            "Durumu 'Hasarlı', 'Kayıp', 'Arşivde' veya 'Bakımda' olarak "
            "işaretlenmiş fiziksel nüshalar burada listelenir."
        )
        info_label.setWordWrap(True)
        layout.addWidget(info_label)

        refresh_button = QPushButton("🔄  Yenile")
        refresh_button.clicked.connect(self.refresh_damaged_lost)
        layout.addWidget(refresh_button)

        self.damaged_lost_table = QTableWidget()
        self.damaged_lost_table.setColumnCount(6)
        self.damaged_lost_table.setHorizontalHeaderLabels([
            "ID",
            "Kitap No",
            "Kitap",
            "ISBN",
        ...
        self.damaged_lost_table.setSelectionBehavior(
            QAbstractItemView.SelectRows
        )
        self.damaged_lost_table.setEditTriggers(
            QAbstractItemView.NoEditTriggers
        )
        self.damaged_lost_table.horizontalHeader().setSectionResizeMode(
            QHeaderView.Stretch
        )
        layout.addWidget(self.damaged_lost_table)

        status_group = QGroupBox(
            "Seçili Nüshanın Durumunu Güncelle"
        )
        ...
        self.damaged_lost_status_combo = QComboBox()
        self.damaged_lost_status_combo.addItems([
            "Mevcut",
            "Hasarlı",
            "Kayıp",
            "Bakımda",
        ...
        update_status_button = QPushButton(
            "Durumu Güncelle"
        )
        update_status_button.clicked.connect(
            self.update_damaged_lost_status
        )

        status_layout.addWidget(
            self.damaged_lost_status_combo
        )
        status_layout.addWidget(
    ...
    def refresh_damaged_lost(self):

        copies = self.db.get_damaged_or_lost_copies()

        self.damaged_lost_table.setRowCount(
            len(copies)
        )
        ...
        for r, copy in enumerate(copies):
            ...
            for c, value in enumerate(values):
                self.damaged_lost_table.setItem(
                    r,
                    c,
                    QTableWidgetItem(str(value))
                )

    def update_damaged_lost_status(self):

        if not require_active_user(self.db, self):
            return

        row = self.damaged_lost_table.currentRow()

        if row < 0:
            QMessageBox.warning(
                self,
                "Uyarı",
                "Durumu güncellenecek nüshayı seçin."
            )
            return

        copy_id = int(
            self.damaged_lost_table.item(row, 0).text()
        )

        new_status = self.damaged_lost_status_combo.currentText()

        try:
            self.db.update_copy_status(copy_id, new_status)
    ...
    def create_activity_log_tab(self):
        ...
        info_label = QLabel(
            "Kitap/üye ekleme, ödünç verme, iade alma, arşivleme "
            "gibi tüm işlemlerin kaydı (ayrı bir veritabanında tutulur)."
        )
        info_label.setWordWrap(True)
        layout.addWidget(info_label)
        ...
        self.activity_log_filter_edit = QLineEdit()
        self.activity_log_filter_edit.setPlaceholderText(
            "Kullanıcı adı veya işlem metninde ara..."
        )
        self.activity_log_filter_edit.returnPressed.connect(
            self.refresh_activity_log
        )

        filter_button = QPushButton("Filtrele")
        filter_button.clicked.connect(self.refresh_activity_log)

        refresh_button = QPushButton("🔄  Yenile")
        refresh_button.clicked.connect(self.refresh_activity_log)

        filter_layout.addWidget(self.activity_log_filter_edit)
        ...
        self.activity_log_table.setColumnCount(5)
        ...
        self.activity_log_table.setEditTriggers(
            QAbstractItemView.NoEditTriggers
        )
    ...
    def refresh_activity_log(self):
        ...
        filter_text = self.activity_log_filter_edit.text().strip().lower()

        # Kullanıcıları önceden tek seferde çekip id -> isim eşlemesi
        # oluşturuyoruz (loglar ayrı bir veritabanında olduğu için
        # kullanıcı bilgisiyle doğrudan JOIN yapılamıyor).
        users_by_id = {
            user["id"]: user
        ...
        for log_row in logs:
            ...
            user = users_by_id.get(log_row["user_id"])

            if user:
                user_display = f"{user['username']} (No: {user['user_number']})"
            else:
                user_display = "— Kayıtsız —"

            if filter_text:
                haystack = (
                    user_display + " " + (log_row["action"] or "")
                ).lower()
            ...
            rows.append((
                display_datetime(log_row["created_at"]),
                user_display,
                log_row["action"] or "",
                log_row["target_type"] or "-",
                log_row["target_id"] if log_row["target_id"] is not None else "-"
            ))

        self.activity_log_table.setRowCount(
            len(rows)
        )

        for r, values in enumerate(rows):
            for c, value in enumerate(values):
                self.activity_log_table.setItem(
                    r,
                    c,
                    QTableWidgetItem(str(value))
                )
    ...
    def create_settings_tab(self):
        ...
        users_layout.addWidget(
            QLabel(
                ...
                "sol menüden 'Kullanıcı Girişi' ile numara girilerek "
                "seçilir."
            )
        )
        ...
        self.new_user_number_edit = QLineEdit()
        self.new_user_number_edit.setPlaceholderText(
            "Kullanıcı Numarası..."
        )
        self.new_user_number_edit.returnPressed.connect(self.save_new_user)

        self.new_user_name_edit = QLineEdit()
        self.new_user_name_edit.setPlaceholderText(
            "Ad Soyad..."
        )
        self.new_user_name_edit.returnPressed.connect(self.save_new_user)

        add_user_button = QPushButton("Kullanıcı Ekle")
        add_user_button.clicked.connect(self.save_new_user)

        add_user_row.addWidget(self.new_user_number_edit)
        add_user_row.addWidget(self.new_user_name_edit)
    ...
    def save_new_user(self):
        ...
        user_number = self.new_user_number_edit.text().strip()
        name = self.new_user_name_edit.text().strip()

        if not user_number or not name:
            QMessageBox.warning(
                self,
                "Uyarı",
                "Kullanıcı numarası ve adı boş olamaz."
            )
            return

        try:
            self.db.add_user(user_number, name)
            self.new_user_number_edit.clear()
            self.new_user_name_edit.clear()
            self.refresh_users_everywhere()
    ...
    def delete_selected_user(self):
        ...
        if row < 0:
            QMessageBox.warning(
                self,
                "Uyarı",
                "Silinecek (pasifleştirilecek) kullanıcıyı seçin."
            )
            return

        user_number = self.users_table.item(row, 0).text()
        user_name = self.users_table.item(row, 1).text()

        users = self.db.get_all_users()
        target_user = next(
            (u for u in users if str(u["user_number"]) == user_number),
            None
        )
        ...
        answer = QMessageBox.question(
            self,
            "Kullanıcıyı Sil",
            f"'{user_name}' ({user_number}) pasifleştirilecek "
            "(kalıcı olarak silinmeyecek). Devam edilsin mi?"
        )
        ...
        try:
            self.db.deactivate_user(target_user["id"])
            self.refresh_users_everywhere()
    ...
    def refresh_users_table(self):
        ...
        for row, user in enumerate(users):
            self.users_table.setItem(
                row, 0, QTableWidgetItem(user["user_number"] or "")
            )
            self.users_table.setItem(
                row, 1, QTableWidgetItem(user["username"] or "")
            )
            self.users_table.setItem(
                row, 2, QTableWidgetItem(user["role"] or "")
            )
            self.users_table.setItem(
                row, 3, QTableWidgetItem(user["status"] or "")
            )
    ...
    def tab_changed(self, index):
        ...
        elif widget == self.damaged_lost_tab:
            self.refresh_damaged_lost()

        elif widget == self.activity_log_tab:
            self.refresh_activity_log()
